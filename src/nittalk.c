...
void server1(int num){                                                                 /// Creates a function to receive the file
    ...
    if(bind(server_id,(struct sockaddr*)&server_addr,sizeof(server_addr))==-1){         /// This first takes the server_id plugs in with the IP address and the port no. It uses type cast bevause it is an univtsal function and it should accept all family of addresses
    ...
    if(listen(server_id,5)==-1){                                                       /// This turns the socket on and wait for the incoming connections, with maximum 5 connections at a time
        perror("listen");
        close(server_id);
        return;
    }
    ...
    if(recv(client_socket,&client_pub_key,sizeof(client_pub_key),0)<=0){               /// The sender's public key is sent and it is stored in client_pub_key
        perror("recv");
        close(client_socket);
        close(server_id);
        return;
    ...
    if(send(client_socket,&public_key1,sizeof(public_key1),0)==-1){                    /// The receiver's public key is sent to the sender
        perror("send");
        close(client_socket);
        close(server_id);
        return;
    ...
    file_header received_file = {0};                                                   /// struct received_file is created from nittalk.h file

    if(recv(client_socket,&received_file,sizeof(received_file),0)<=0){                 /// The incoming bytes is stored in the received_file variable which was received via the client_socket port
             perror("recv");
            close(client_socket);
            close(server_id);
            return;
    ...
    if(strncmp(received_file.ver_bytes,"NIT\x00",4)!=0){                               /// If the initial 4 bytes of the received data is not NIT 
        ...
        uint32_t num1=received_file.file_size;                                         /// The file size is stored in a variable     
        ...
        while(total_bytes_received<received_file.file_size){                           /// As long as the total bytes received is lesser than the file size     
            ...
            if(bytes_read==0){                                                         /// If nothing is copied means, the loop is broken 
                ...
                if(fwrite(temp,1,bytes_read,fp)!=bytes_read){                          /// The bytes from temp array is stored in the file
                    perror("fwrite");
                    fclose(fp);
                    close(client_socket);
                    close(server_id);
...
void client1(const char *target_ip_add, const char *filename,int port){                /// Creates a function to send the file
    ...
    if(inet_pton(AF_INET,target_ip_add,&target_addr.sin_addr)<=0){                     /// This converts the human readable IP address to binary form and stores in the struct 
        perror("inet_pton");
        close(client_id);
        return;
    ...
    file_header send_file = {0};                                                       /// A send_file struct is created
    ...
    if(send(client_id,&public_key,sizeof(public_key),0)<=0){                           /// The public key of the sender is sent to the receiver 
        perror("send");
        close(client_id);
        return;
    }  
    ...
    if(recv(client_id, &server_pub_key, sizeof(server_pub_key), 0)<=0){                /// The public key of the receiver is stored in the server_pub_key   
        perror("recv");
        close(client_id);
        return;
    }                              
    ...
    if(send(client_id,&send_file,sizeof(send_file),0)<=0){                             /// This sends the details of file header across the server and tells how many bites of file size to be expected
        perror("send");
        close(client_id);
        return;
    }                                    
    ...
    while(1){
        ...
        size_t bytes_read=fread(temp,1,sizeof(temp),source_file);                      /// The variable bytes_read is created so that to keep track of the bytes copied from source_file to temp with 1 byte at a time
        ...
        if(bytes_read==0){                                                             /// If no bytes are copied..... 
            ...
            if(send(client_id,temp,bytes_read,0)<=0){                                  /// The bytes are sent across the server  
                perror("send");
                fclose(source_file);
                close(client_id);
                return;
